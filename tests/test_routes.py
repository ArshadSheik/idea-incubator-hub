...
from models.models import db, User, Idea, Vote, Comment, Collaboration, Task, Bookmark, Notification
...
class TestIdeaDetail:
    ...
    def test_idea_detail_returns_200(self, auth_client, sample_idea):
...
class TestTaskEndpoints:
    ...
    def test_create_task_returns_201(self, auth_client, sample_idea):
    ...
    def test_create_task_without_title_rejected(self, auth_client, sample_idea):
        r = auth_client.post(f"/api/ideas/{sample_idea}/tasks",
                             json={"title": "", "priority": "medium"})
        assert r.status_code == 400

    def test_update_task_status(self, auth_client, app, sample_idea):
        r = auth_client.post(f"/api/ideas/{sample_idea}/tasks",
                             json={"title": "A task", "status": "todo"})
        task_id = json.loads(r.data)["task_id"]

        r2 = auth_client.put(f"/api/ideas/{sample_idea}/tasks/{task_id}",
                              json={"status": "done"})
    ...
    def test_delete_task(self, auth_client, app, sample_idea):
        ...
        r2 = auth_client.delete(f"/api/ideas/{sample_idea}/tasks/{task_id}")
        assert r2.status_code == 200
